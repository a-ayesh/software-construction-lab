...
public class RulesOf6005 {
     ...
     */
	    ...
	    // You can use the code if you wrote it yourself or it's implemented.
	    if (writtenByYourself || implementationRequired) {
	        return true;
	    }
	    
	    // If the code is not written by yourself, you can't use it if it's not available to others
	    // or if it was written as course work (even in previous terms) and not properly cited.
	    if (!writtenByYourself && (!availableToOthers || writtenAsCourseWork && !citingYourSource)) {
	        return false;
	    }
	    
	    // If none of the above conditions are met, you can't use the code.
	    return false;
	}
